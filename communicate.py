...
class Communicate:
    ...
    async def __stream(self):
        """
        Stream audio and metadata messages from Edge TTS with correct headers.
        """
        import json, time, random, string, ssl, aiohttp

        url = "wss://speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1"

        # Microsoft Edge requires these headers to return audio
        headers = {
        ...
        # Random request ID like Edge browser does
        request_id = "".join(random.choices(string.ascii_letters + string.digits, k=16))
        params = f"?TrustedClientToken=undefined&ConnectionId={request_id}"
        full_url = url + params
        ...
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(
                ...
                # Send configuration and SSML payloads
                config = {
                    "context": {
                        "synthesis": {
                            "audio": {
                ...
                await ws.send_str("Path: speech.config\r\nX-RequestId:" + request_id + "\r\nContent-Type: application/json\r\n\r\n" + json.dumps(config))
                ssml = self._create_ssml()
                await ws.send_str("Path: ssml\r\nX-RequestId:" + request_id + "\r\nContent-Type: application/ssml+xml\r\n\r\n" + ssml)
